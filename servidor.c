...
int conectar_cliente(const char* ip_servidor, int porta) {
    ...
    if (inicializar_protocolo_cliente(&estado_servidor, ip_servidor, PORTA, porta, INTERFACE_NAME) < 0) {
...
int inicializar_servidor() {
    // Definir interface de rede (você pode precisar ajustar isso)
    const char* interface = INTERFACE_NAME; // ou "wlan0", "enp0s3", etc.
    
    // Inicializar protocolo do servidor
    if (inicializar_protocolo_servidor(&estado_servidor, interface) < 0) {
        fprintf(stderr, "Erro ao inicializar protocolo do servidor\n");
        return -1;
    }
